 ...
 * - **系统栏占位**（底 inset > 0）⇒ 用真实 inset（= 系统栏底 ∪ 挖孔底），与改动前逐像素相同；
 *   底 inset 小于 24dp 的可见态**不得被抬高**（这是「可见支」的判别力所在）；边缘上拨**瞬态唤出**的那几秒是否也走
 *   这一支，取决于平台在瞬态显示期间是否把 `systemBars` 底报为非 0——**本机未验证（真机观察项）**；
