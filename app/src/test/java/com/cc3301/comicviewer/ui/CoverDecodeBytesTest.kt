 ...
 * 封面解码的真实位图字节数（票 #81）。
 *
 * [CoverDecodeTest] 锁的是纯函数口径（计划里的字节数），这里补上端到端的一环：真的过
...
class CoverDecodeBytesTest {
    ...
    /** 网格 2 列在 3.0 密度下的桶（= 票面验收里的 512px） */
    private val gridTarget = CoverDecode.targetWidthPx(165f * 3f)
    ...
    fun `800x8000 封面的位图字节数与 3-4 封面同量级`() {
        ...
        val longBytes = long!!.asAndroidBitmap().allocationByteCount
        val normalBytes = normal!!.asAndroidBitmap().allocationByteCount
        assertTrue(
        ...
        // 绝对上界：可见带的像素量级只与**显示盒**有关（非源高），下栏长条封面因此不再随源高增长
        val boxBytes =
            gridTarget * (gridTarget * CoverLayout.GRID_CELL_ASPECT).toInt() * CoverDecode.RGB_565_BYTES_PER_PIXEL
    ...
    fun `超宽封面不整张 1-1 解码`() {
        ...
        assertNotNull(wide)
        val wideBytes = wide!!.asAndroidBitmap().allocationByteCount
        val normalBytes = decode("normal", png(800, 1067), gridTarget, grid)!!.asAndroidBitmap().allocationByteCount
