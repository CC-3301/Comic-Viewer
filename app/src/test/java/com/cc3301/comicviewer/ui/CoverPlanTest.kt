 ...
 *
 * **期望值不自比较**（r2 b2）：键串写**字面量**、桶用**独立重算**（整数算术，不走生产函数）、通路判据写
 * **字面量布尔**——拿被测的生产函数当期望值等于用例永不失败。桶的口径（向上取整到 [CoverDecode.BUCKET_PX]
 * 的倍数）在出货几何下的两个字面量锚点是 448（156dp × 2.75）与 160（56dp × 2.75）。
 */
class CoverPlanTest {
