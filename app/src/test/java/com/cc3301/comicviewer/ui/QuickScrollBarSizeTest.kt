 ...
 *
 * **本轮的用例为什么写成两层（上一版翻车的原因见 `.implement-pro/60/evidence-impl.md §23①`）**：
 * r4/r5 的用例只钉了「空档内两侧相等」这个**自洽关系** —— 它与本体在屏上的绝对位置无关，空档取哪个值
 * 都成立，所以真机说「偏左」时用例照样全绿。本轮**同时**钉：
 *
 ...
 *
 * 判别力（实测过，见证据「先红后绿」）：把 [quickScrollBarEdgeGap] 换成「贴屏幕侧固定」的
 * `10.dp − barWidth / 2`（r6/r8 的写法）⇒ [空档 20dp 时本体右缘离屏缘 7dp] 仍绿，但
 * [空档 68dp 时本体右缘离屏缘 31dp] 与 [两个空档下的抓取带宽] 变红（固定写法恒为 7dp / 13dp）；
 * 反过来只钉关系不钉绝对量时上面那组仍然全绿。两条一起才挡得住两个方向的漂移。
 * 本体宽改回 4dp / 下限退回 24dp / 水平右留白改回 12dp 也会变红；[quickScrollBarStripWidth] 丢了
 * 「+ 本体宽」时 [本体整体落在抓取带内] 与两条「不侵入右留白」变红。
 *
...
class QuickScrollBarSizeTest {
     ...
     * **已知限制**：inset 大于本体占位（13dp）时本体落在系统 inset 区内（48dp 导航栏带 0–48dp 里占 31–37dp），
     * 是否被系统栏盖住 / 命中被接管**尚不确定**，由真机验收判（本地判不了，不造探针）。
     */
    @Test
     ...
     *
     * 判别力：去掉 `coerceAtLeast(0.dp)` 时前两条变红（`(4 − 6) / 2` = −1dp）；空档 7dp 那条在
     * `gap − barWidth` 夹取（旧实现）下给 1dp、在居中式下给 0.5dp，两边都 ≤ 空档 − 本体，因此它单钉
     * 「不压内容」而不钉具体值（具体值由上面两条绝对量用例钉）。
     */
    @Test
