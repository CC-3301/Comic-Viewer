 ...
 *
 * 本地那条（票 #40，验收标准 4）要求「可直接照着操作」：界面上的修法是删掉这条连接再重新授权文件夹，
 * 所以提示里必须点出删除动作、点明入口在首页「本地」、并说明删完再重新添加文件夹。这三样都由下面
 * 那条精确断言钉住（不再另加 `contains` 弱断言：它们与精确断言重叠，零新增可证伪性）。
 */
class LoadFailureHintTest {

    @Test
    fun `本地提示授权失效并给出删除动作 其余来源统一提示检查网络或服务器`() {
        assertEquals(
