...
/**
 * `ui` 包的 Robolectric 组合测量脚手架（票 #115）：把三处同构的「起 [ComponentActivity] → 挂 [ComposeView]
 * → measure → layout → idle」合成一份（ReaderOverlayInsetsTest 票 #61 / ReaderMenuTitleTest 票 #67 /
 * TopBarTitleTest 票 #79），PreviewStrip 两处（PreviewStripCenterTest / PreviewStripLayoutTest）的布局驱动
 * 也是同一段。换测量方式（改 idle 口径、换测量驱动）从此只改这一处，不再各处漏改。
 *
