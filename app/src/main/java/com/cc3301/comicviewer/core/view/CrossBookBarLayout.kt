 ...
 * 的黑六成再加深一档：真机反馈「还是有点透，不够黑」），高 [BAR_HEIGHT_DP]，文字 [LABEL_SP]，
 ...
 *
 * ## 命中格 = 视觉格整格（r2 根因修正）
 * 条的**可见范围**（有黑底的那一块）就是整块条面：内容带 [BAR_HEIGHT_DP] + 底部的底部避让。
 * 命中层因此铺满**整块条面**（宿主里是 `fillMaxSize()`），不是只铺到内容带——后者会在按钮格内部
 * 留出一截死区（沉浸态 24dp ≈ 格高的 27%，且正是手指从触摸区直直向下移到底时落点所在），
 * 真机表现就是「点击左右选区有时候无效」。横坐标仍走 [confirmsAt] 的三等分，所以命中格与触摸区逐像素对齐。
 *
 * ## 命中判定为什么复用触摸区
...
internal object CrossBookBarLayout {
    ...
    /** 内容带高（dp）：文案所在那一带的高；命中格比它高（= 整块条面，见 [bandHeightDp]） */
    const val BAR_HEIGHT_DP: Float = 64f

    /**
     * 条面（黑底）的不透明度：**黑七成八**（r2 口径；批次 6 是 0.6，真机反馈「还是有点透，不够黑」）。
     * 画面从底下透出来一档、被压暗——不是纯黑（1f）、不是全透明（0f）、也不是毛玻璃。
     * 加深还有第二个作用：条上文字的对比度跟着上去（见 `ui/CrossBookBarStyle.crossBookBarLabelContrast`）。
     */
    const val BAR_ALPHA: Float = 0.78f
