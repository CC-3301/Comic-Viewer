...
internal object CoverDecode {
     ...
     *    （800×8000 → 峰值 5.9MB vs 2.4MB，退回区域带）；源高宽比大而源宽不够大时它与整图子采样基本持平
     *    （2200×20000 → 峰值 5.47MB vs 5.50MB，保留位图却从 5.5MB 降到 0.67MB），这条仍轻过整图子采样、
     *    因此照常选中裁剪解码。**不许 clamp 目标尺寸**——目标尺寸与 crop 矩形是一套映射（矩形在里居中），
     *    改小就取不到那条带了。
     *
