 ...
 *
 * **读数口径（票 #124）**：累计值**没有复位点**（本对象的存活期就是进程的存活期），单看 `overBudgetTotal`
 * 分不清「刚跑的这 10 次」与「进程以来全部」。因此每行还带 `transitions`（本对象开过的过渡窗口序号，从 1 起）：
 * 取数时**重启 APP**（`log.tag.ComicViewerPerf` 的开关同样要重启才生效）后直接连做 10 次进出阅读器
 * （= 20 次过渡），**读 `transitions=20` 那一行的 `overBudgetTotal`**，不要手工相加、也不要读最后一行。
 * 中途插了其它导航（抽屉/层级）时，序号按实际拍数往后推。
 *
