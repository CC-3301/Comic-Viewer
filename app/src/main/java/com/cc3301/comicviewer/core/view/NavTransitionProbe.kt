 ...
 * （`log.tag.ComicViewerPerf` 的开关同样要重启才生效），先做一次进出并**记下那一行的序号 X**，再连做 10 次
 * 进出阅读器（= 20 次过渡），**读 `transitions=X+20` 那一行的 `overBudgetTotal`**，不要手工相加、也不要读
 * 最后一行。**序号起点不是 1**：冷启动落地本身也计拍（`AppNav` 的 `navigate(HOME)` 与逐层
