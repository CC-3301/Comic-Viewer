 ...
 * - **系统栏底 inset > 0**（栏可见，含边缘上拨**瞬态唤出**的那几秒）：用真实 inset（= 系统栏底 ∪ 挖孔底），
 *   与改动前逐像素相同——列表界面照旧由 Scaffold 自己消费、不双重内缩；而瞬态唤出的系统栏/胶囊（约 48dp）
 *   也靠这支自动把浮层抬到真实高度之上，不会压住菜单最底一行；
...
internal object ReaderOverlayLayout {
     ...
     * 边缘上拨瞬态唤出的系统栏/胶囊（约 48dp）走的是「真实 inset」那一支，也不靠这个常量兜。
     */
     ...
     *
     * @param systemBarsBottomPx `WindowInsets.systemBars` 的底部 inset（px）；栏隐藏时为 0、瞬态唤出时 > 0
